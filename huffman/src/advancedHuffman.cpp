/**
 * Соревнование
 * На базе задачи 4 предлагается разработать оптимальный алгоритм сжатия данных.
 * Это может быть как усовершенствованный алгоритм Хаффмана, так и любой другой алгоритм.
 */
#include <string>
#include <queue>
...
class AdvancedHuffman {
 public:
  AdvancedHuffman() = default;
  ...
  std::string encode(std::string &originalString) {
    ...
    bool charIsChecked[ALPHABET_SIZE] = {false};
    int frequenciesChar[ALPHABET_SIZE][ALPHABET_SIZE] = {0};
    int alphabetSize = 0;
    Node *charMap[ALPHABET_SIZE][ALPHABET_SIZE] = {nullptr};
    std::vector<Node*> additionalNodes;

    for (unsigned long long currentCharPos = 0; currentCharPos < originalString.size(); currentCharPos++) {
      if (!charIsChecked[(unsigned char)originalString[currentCharPos]]) {
        charIsChecked[(unsigned char)originalString[currentCharPos]] = true;
        alphabetSize++;

        for (unsigned long long nextCharPos = currentCharPos; nextCharPos < originalString.size(); nextCharPos++) {
          if ((unsigned char)originalString[nextCharPos] == (unsigned char)originalString[currentCharPos]) {
            if (nextCharPos + 1 < originalString.size()) {
              frequenciesChar[(unsigned char)originalString[currentCharPos]][(unsigned char)originalString[nextCharPos + 1]]++;
            }
          }
        }
      }
    ...
    Buffer buffer;
    buffer.addBits(alphabetSize);
    result += buffer.getBuffer();
    buffer.clear();

    for (int currentMainChar = 0; currentMainChar < ALPHABET_SIZE; currentMainChar++) {
      std::priority_queue<Node*, std::vector<Node*>, Compare<Node*> > heap;
      std::vector<Node*> usingChars;

      for (int charIndex = 0; charIndex < ALPHABET_SIZE; charIndex++) {
        if (frequenciesChar[currentMainChar][charIndex] != 0) {
          Node *newNode = new Node(frequenciesChar[currentMainChar][charIndex], charIndex);
          heap.push(newNode);
          charMap[currentMainChar][charIndex] = newNode;
        }
      }
      ...
      if (!heap.empty()) {
        ...
        root = buildHelpTree(heap, additionalNodes);
        calculateHelpCode(root, 0, usingChars);
        compressedTreeRoot = buildCompressedTree(usingChars, additionalNodes);
        calculateCompressedCode(compressedTreeRoot, "");

        result += (unsigned char)currentMainChar;
        buffer.addBits(usingChars.size());
        result += buffer.getBuffer();
        buffer.clear();
    ...
    for (unsigned long long currentCharIndex = 1; currentCharIndex < originalString.size(); currentCharIndex++) {
      std::string compressedCode = charMap[(unsigned char)originalString[currentCharIndex - 1]][(unsigned char)originalString[currentCharIndex]] -> compressedCode;
      buffer.addBits(compressedCode);
    }
    ...
    if (result.size() > originalString.size()) {
      ...
      buffer.clear();
      buffer.addBits(0);
      newResult += buffer.getBuffer();
      newResult += originalString;
      return newResult;
    } else {
      return result;
    }
  ...
  std::string decode(std::string &compressedString) {
    ...
    Reader reader(compressedString);
    if (reader.isFail()) {
      return compressedString.substr(4, compressedString.size());
    }

    Node **compressedForest = reader.getCompressedForest();
    std::string compressedPart = reader.getCompressedPart();

    Decoder decoder(compressedPart[0], compressedForest);

    for (unsigned long long charIndex = 1; charIndex < compressedPart.length() - 2; charIndex++) {
      decoder.decompressByte(compressedPart[charIndex]);
    }

    decoder.decompressByte(compressedPart[compressedPart.length() - 2], 8 - compressedPart[compressedPart.length() - 1]);

    std::string result = compressedPart[0] + decoder.getOutputBuffer();
 ...
 private:
  ...
  class Reader {
   public:
    explicit Reader(std::string &inputString) {
      int mainCharsCount = readInt(0, inputString);
      if (mainCharsCount == 0) {
        flgStop = true;
        return;
      }
      ...
      while (mainCharsCount > 0) {
        std::vector<Node*> usingChars;
        auto currentMainChar = (unsigned char)inputString[pos];
        pos++;

        int additionalCharCount = readInt(pos, inputString);
        pos += 4;

        for (unsigned long long byteIndex = pos; byteIndex < additionalCharCount * 2 + pos; byteIndex += 2) {
          Node *newNode = new Node;
          newNode -> originalCode = inputString[byteIndex];
          newNode -> codeLength = inputString[byteIndex + 1];
          usingChars.push_back(newNode);
        ...
        pos += additionalCharCount * 2;

        Node *currentCompressedTree = buildCompressedTree(usingChars,additionalNodes);
        compressedForest[currentMainChar] = currentCompressedTree;

        mainCharsCount--;
      }

      compressed = inputString.substr(pos, inputString.length());
    }

    Node **getCompressedForest() {
      return compressedForest;
    }

    std::string getCompressedPart() {
      return compressed;
    }
   ...
   private:
    Node *compressedForest[ALPHABET_SIZE] = {nullptr};
    std::vector<Node*> additionalNodes;
    std::string compressed;
    bool flgStop = false;
  };
  ...
  class Decoder {
   public:
    explicit Decoder(unsigned char initByte, Node **compressedForest) {
      forest = compressedForest;
      currentNode = compressedForest[initByte];
    }
    ...
    void decompressByte(unsigned char byte, int shift = 8) {
      for (int bitIndex = 0; bitIndex < shift; bitIndex++) {
        ...
        if ((currentNode -> rightChild == nullptr) && (currentNode -> leftChild == nullptr)) {
          buffer += currentNode -> originalCode;
          currentNode = forest[(unsigned char)currentNode -> originalCode];
        }
  ...
  Node *buildHelpTree(std::priority_queue<Node*, std::vector<Node*>, Compare<Node*> > &heap, std::vector<Node*> &additionalNodes) {
    Node *root;
    bool aloneSymbol = heap.size() == 1;
  ...
  static Node *buildCompressedTree(std::vector<Node*> alphabet, std::vector<Node*> &usedNodes) {
    std::priority_queue<Node*, std::vector<Node*>, CompareCodeLength<Node*> > heapForCompressingTree(alphabet.begin(), alphabet.end());
    bool aloneSymbol = heapForCompressingTree.size() == 1;
    Node *compressedTreeRoot = nullptr;
...
int main() {
  AdvancedHuffman huffman;
  SimpleHuffman simpleHuffman;

  std::string in = "";
  std::ifstream originalText("../huffman/debug/texts/original/alice", std::ios::binary);
  std::ofstream decodedText("../huffman/debug/texts/decoded/alice");
  ...
  std::string compressed = huffman.encode(in);
  std::string simpleCompressed = simpleHuffman.encode(in);

  std::string decoded = huffman.decode(compressed);
  std::string simpleDecoded = simpleHuffman.decode(simpleCompressed);

  decodedText << decoded;
  std::cout << compressed.size() << " " << simpleCompressed.size() << " " << in.size() << std::endl;
  std::cout << decoded.size() << " " << simpleDecoded.size() << " " << in.size() << std::endl;
  return 0;
}
...
void Encode(IInputStream& original, IOutputStream& compressed) {
  ...
  AdvancedHuffman advancedHuffman;
  compressedText = advancedHuffman.encode(originalText);
...
void Decode(IInputStream& compressed, IOutputStream& original) {
  ...
  AdvancedHuffman advancedHuffman;
  originalText = advancedHuffman.decode(compressedText);
